...
Module ModPrincipal
    ...
    Public Function NivelAcesso(fCodusuario As Integer, fCodModulo As Integer, fNomeOpcao As String, fPrincOpcao As String) As Integer
        ...
        If nNivelAcesso = 0 Then
            'Verificar a permissão pelo grupo
            cQuery = "SELECT SI002_NIVACE FROM ESI002, ESI000 where SI002_CODGRU=SI000_CODGRU" & _
                " and SI000_CODUSU = " & fCodusuario & " and SI002_CODOPC = " & nCodOpcao
    ...
    Public Function getLoginUsuario(ByVal fCodUsuario As Integer) As Integer
        Dim cQuery As String
        Dim dtUsuario As DataTable = New DataTable("ESI000")
    ...
    'Usuario Atual
    Public Function getLoginaAcessoUsuario(ByVal fCodUsuario As Integer) As String
        Dim cQuery As String
        Dim dtUsuario As DataTable = New DataTable("ESI000")
    ...
    'Classe para leitura da tabela Armazéns
    Public Function LerCod_Armazens(fClass_Arm As String, Optional ByRef fNomeArmazens As String = "") As String
        Dim cQuery As String
        Dim dtLerCodArmazens As DataTable = New DataTable("EES003")

        cQuery = "SELECT ES003_CODARM, ES003_DESARM FROM EES003 where ES003_CODARM = '" & fClass_Arm & "'"

        Using daTabela As New OleDbDataAdapter()
            daTabela.SelectCommand = New OleDbCommand(cQuery, g_ConnectBanco)

            ' Preencher o DataTable 
            daTabela.Fill(dtLerCodArmazens)
            If dtLerCodArmazens.Rows.Count > 0 Then
                fNomeArmazens = dtLerCodArmazens.Rows(0).Item("ES003_DESARM")
                Return dtLerCodArmazens.Rows(0).Item("ES003_CODARM")
            Else
                Return ""
            End If
        End Using

        dtLerCodArmazens.Clear()

    End Function

    'Classe para leitura da tabela Tipo de Produto
    Public Function LerCod_TipdeProd(fClass_TipdeProd As Integer, Optional ByRef fNomeTipdeProd As String = "") As Double
        Dim cQuery As String
        Dim dtLerCodTipdeProd As DataTable = New DataTable("EES001")

        cQuery = "SELECT ES001_TIPPRO, ES001_DESTIP FROM EES001 where ES001_TIPPRO =" & fClass_TipdeProd

        Using daTabela As New OleDbDataAdapter()
            daTabela.SelectCommand = New OleDbCommand(cQuery, g_ConnectBanco)

            ' Preencher o DataTable 
            daTabela.Fill(dtLerCodTipdeProd)
            If dtLerCodTipdeProd.Rows.Count > 0 Then
                fNomeTipdeProd = dtLerCodTipdeProd.Rows(0).Item("ES001_DESTIP")
                Return dtLerCodTipdeProd.Rows(0).Item("ES001_TIPPRO")
            Else
                Return 0
            End If
        End Using

        dtLerCodTipdeProd.Clear()

    End Function

    'Função Preenchimento de Tipo de Produtos
    Public Function LerClasse_TipodeProd(fCodTipodePro As Integer, Optional ByRef fNomeTipodeProd As String = "") As String
        Dim cQuery As String
        Dim dtLerCodTipodeProd As DataTable = New DataTable("EES001")

        cQuery = "SELECT ES001_TIPPRO, ES001_DESTIP FROM EES001 where ES001_TIPPRO = " & fCodTipodePro

        Using daTabela As New OleDbDataAdapter()
            daTabela.SelectCommand = New OleDbCommand(cQuery, g_ConnectBanco)

            ' Preencher o DataTable 
            daTabela.Fill(dtLerCodTipodeProd)
            If dtLerCodTipodeProd.Rows.Count > 0 Then
                fNomeTipodeProd = dtLerCodTipodeProd.Rows(0).Item("ES001_DESTIP")
                Return dtLerCodTipodeProd.Rows(0).Item("ES001_TIPPRO")
            Else
                Return ""
            End If
        End Using
        dtLerCodTipodeProd.Clear()

    End Function

    'Função Preenchimento Armazéns
    Public Function LerClasse_Armazens(fCodArmazens As String, Optional ByRef fNomeArmazens As String = "") As String
        Dim cQuery As String
        Dim dtLerCodArmazens As DataTable = New DataTable("EES003")

        cQuery = "SELECT ES003_CODARM, ES003_DESARM FROM EES003 where ES003_CODARM = '" & fCodArmazens & "'"
    ...
    Public Function ProxSeq_Unidade(fClaUnidade As String) As String
        Dim cQuery As String = ""
        Dim dtProxSeqUni As DataTable = New DataTable("EUN000")
        Dim nPos As Integer
        Dim sProxSeq As String = ""

        If fClaUnidade = "00.00.00.00" Then 'CM
            cQuery = "right(UN000_CLAUNI,8)='00.00.00'"
            nPos = 1
        ElseIf Microsoft.VisualBasic.Right(fClaUnidade, 8) = "00.00.00" Then 'CC
            cQuery = "right(UN000_CLAUNI,5)='00.00' and left(UN000_CLAUNI,2) = '" & Microsoft.VisualBasic.Left(fClaUnidade, 2) & "'"
            nPos = 4
        ElseIf Microsoft.VisualBasic.Right(fClaUnidade, 5) = "00.00" Then 'CP
            cQuery = "right(UN000_CLAUNI,2)='00' and left(UN000_CLAUNI,5) = '" & Microsoft.VisualBasic.Left(fClaUnidade, 5) & "'"
            nPos = 7
        Else 'If Microsoft.VisualBasic.Right(fClaUnidade, 2) = "00" Then
            cQuery = "right(UN000_CLAUNI,2)<>'00' and left(UN000_CLAUNI,8) = '" & Microsoft.VisualBasic.Left(fClaUnidade, 8) & "'"
            nPos = 10
        End If

        cQuery = "SELECT max(UN000_CLAUNI) as UltSeq FROM EUN000 " & IIf(Trim(cQuery) = "", "", "where " & cQuery)
        ...
        Using daTabela As New OleDbDataAdapter()
            ...
            If dtProxSeqUni.Rows.Count > 0 Then
                ...
                Else
                    sProxSeq = "1"
                End If
                sProxSeq = Microsoft.VisualBasic.Mid(fClaUnidade, nPos, 2)
                sProxSeq = Microsoft.VisualBasic.Left(fClaUnidade, nPos - 1) & _
                        (Val(sProxSeq) + 1).ToString("00")
                If nPos < 10 Then
                    sProxSeq += CStr((CInt(sProxSeq) + 1)) & Microsoft.VisualBasic.Right(dtProxSeqUni.Rows(0).Item("UltSeq"), 11 - CInt(nPos) - 1)
                End If
                Return sProxSeq
            Else
                Return ""
    ...
    Public Function LerCargo_Colaborador(fCodigo_Col As Double, Optional ByRef sCodUnidade As Double = 0) As String
        ...
        cQuery = "SELECT EUN003.UN003_CODCOL, EUN003.UN003_NOMCOL, EUN011.UN011_DESOCP, EUN016.UN016_DESMDT, " & _
           ...
           "where UN003_CODCOL = " & fCodigo_Col.ToString
